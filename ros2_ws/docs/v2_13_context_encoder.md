...
## Multi-phase variant (v2_13_v2)

The v3 single-phase encoder is complemented by a v2_13_v2 trained
on a synthetic multi-phase trial generated by
`run_synthetic_multiphase_trial.launch.py`. That trial replaces the
admittance_insertion_node with `synthetic_phase_publisher` and
publishes /task_phase on a scripted schedule (30s MOVE_TO_START ->
20s APPROACH -> 40s SEARCH -> 15s HOVER_ABOVE_HOLE -> 30s INSERT
-> 10s INSERTED -> 10s RETREAT -> 15s ABORT, total 170s). The arm
does NOT execute any controller commands; /task_phase labels are
time-window proxies, not real motor-actuated phases.

The recorded CSV (10 MB, 4305 rows) and parquet
`context_log.parquet` are in
...
ros2 run perception_pipeline v2_13_context_encoder -- \
    ...
    --output-dir diagnostics/perception_pipeline_v2_13_encoder_v2 \
    --epochs 200 --batch-size 64 --lr 0.001 \
    --seed 0 --latent-dim 32
```

yields `train_mse_final ~= 0.0070`, `test_mse_final ~= 0.0061`.
Higher than v1 because the data spans 7 distinct phases
(MOVE_TO_START, APPROACH, SEARCH, HOVER_ABOVE_HOLE, INSERT,
INSERTED, RETREAT, ABORT) and the autoencoder is reconstructing
multi-modal trajectories, not a single operating mode. Test MSE
is below train MSE, so the model generalizes. The 32-dim latent
now captures phase-discriminative structure.
