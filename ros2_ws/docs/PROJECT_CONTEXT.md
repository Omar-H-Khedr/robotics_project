...
Tracking stabilization should now focus on controller/physics configuration, final-pose damping, and high free-space F/T behavior using the command-vs-feedback evidence. A globally slower move-to-start trajectory and repeated same-target hold corrections were both tested and rejected. Do not loosen the no-contact XY gate to hide the error.
