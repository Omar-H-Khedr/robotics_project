...
- The canonical `position_derivative_gain` is now a launch argument and SDF injection point. Three headless Gazebo diagnostics tested it:
  ...
  - All four runs are fail-closed and safe. The D-term plumbing is correctly applied, but the centered-hold p95 actual XY drift remains around `4 mm` and `joint_1` is still the dominant p95 XY contributor. The best 1 mm SEARCH window stays at 3 ticks across all four diagnostics, well below the required 8; the best 2 mm window peaks at 10 ticks only in the combined higher-gain / higher-D-term run. The next iteration should look beyond the position controller gains and D-term (controller rate, controller type, or higher-level feedback shaping).
