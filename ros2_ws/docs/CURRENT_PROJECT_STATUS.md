...
- The canonical `position_derivative_gain` is now a launch argument and SDF injection point. Three headless Gazebo diagnostics tested it:
  ...
  - All three runs are fail-closed and safe. The D-term plumbing is correctly applied, but the centered-hold p95 actual XY drift remains around `4 mm` and `joint_1` is still the dominant p95 XY contributor. The combined higher-gain / higher-D-term run materially widens the best 2 mm SEARCH window, but the best 1 mm window is still 3 ticks and the `0.0010 m` sustained gate cannot be met. The next iteration should look beyond the position controller gains and D-term (controller rate, controller type, or higher-level feedback shaping).
