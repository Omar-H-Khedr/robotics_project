...
- The canonical `position_derivative_gain` is now a launch argument and SDF injection point. Three headless Gazebo diagnostics tested it:
  ...
  - The 2x2 grid of (gain, D-term) at the canonical damping scale is now complete on the diagnostic side. Raising the gain without a D-term amplifies the controller's oscillation around the SEARCH target and is strictly worse than the canonical gain=2000 baseline. The D-term is necessary to make higher gain tractable, and the combined higher-gain / higher-D-term run (v3) is the best 2 mm SEARCH window seen so far. The next iteration should look beyond the position controller gains and D-term (controller rate, controller type, or higher-level feedback shaping).
