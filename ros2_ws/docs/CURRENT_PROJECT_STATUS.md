...
- The canonical `position_derivative_gain` is now a launch argument and SDF injection point. Two headless Gazebo diagnostics tested it at `0.5` and `5.0` with `position_gain:=2000.0` and `joint_damping_scale:=5.0`:
  - `research_baseline_search_derivative_gain_v1` (D=0.5): SEARCH failed closed at final XY `0.0039 m`, max centered-hold p95 actual XY drift `0.003919 m`, max centered-hold p95 joint error `0.008617 rad`, best SEARCH `0.0010 m` window `3` ticks.
  - `research_baseline_search_derivative_gain_v2` (D=5.0): SEARCH failed closed at final XY `0.0052 m` (reporter) / `0.0027 m` (mean), max centered-hold p95 actual XY drift `0.004018 m`, max centered-hold p95 joint error `0.008434 rad`, best SEARCH `0.0010 m` window `3` ticks.
  - Both runs are fail-closed and safe. The D-term plumbing is correctly applied, but the centered-hold p95 actual XY drift remains around `4 mm` and `joint_1` is still the dominant p95 XY contributor. The D-term alone is insufficient; the next iteration should combine it with a different lever (controller rate, controller type, or higher-level feedback shaping).
