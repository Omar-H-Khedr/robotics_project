...
- KUKA model: real-scale `lbr_iisy6_r1300`; the robot is not scaled in the task scene.
...
- Robot spawn in the unified baseline: base frame on the pedestal at `[0.80, -0.75, 0.735]` with yaw `1.5708 rad`, aligned with the table centerline and facing the table.
...
- `urdf/lbr_iisy6_r1300_research_gripper.urdf.xacro`: project-owned KUKA wrapper that includes the project-local KUKA iisy6 R1300 support description and attaches the passive gripper at `flange`.
- `urdf/lbr_iisy3_r760_research_gripper.urdf.xacro`: legacy iisy3 R760 wrapper kept for reproducibility.
...
The gripper is implemented in this package. The project-owned wrapper `lbr_iisy6_r1300_research_gripper.urdf.xacro` reuses the project-local KUKA `lbr_iisy6_r1300` macro and attaches the gripper through the simulated F/T sensor link. This keeps the research tool model versioned with the peg-in-hole workcell while preserving a clear boundary between the task tool and robot support description. The wrapper disables the optional TF-only camera by default; the canonical Gazebo world owns the simulated D405 sensor.
