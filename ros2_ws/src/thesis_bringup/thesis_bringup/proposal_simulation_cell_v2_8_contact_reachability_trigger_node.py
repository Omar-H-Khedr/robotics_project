"""Contact reachability and trigger validation for proposal_simulation_cell_v2_8."""
...
class ProposalSimulationCellV28ContactReachabilityTriggerNode(Node):
    """Run a bounded Gazebo-only contact reachability validation."""

    def __init__(self) -> None:
        super().__init__("proposal_simulation_cell_v2_8_contact_reachability_trigger_node")
        self.declare_parameter("config_path", "")
        self.declare_parameter("output_dir", "diagnostics/proposal_simulation_cell_v2_8")
        ...
        self._output_dir = Path(
            ...
            or validation.get("output_dir", "diagnostics/proposal_simulation_cell_v2_8")
        ).expanduser()
        self._output_dir.mkdir(parents=True, exist_ok=True)

        self._simulation_engine = str(task.get("simulation_engine", "gazebo"))
        self._task_sequence_type = str(task.get("task_sequence_type", "contact_reachability_and_trigger_validation"))
        ...
        self._success_status = str(validation.get("status_success_contact", "contact_reachability_validated"))
        self._no_contact_status = str(validation.get("status_no_contact", "contact_reachability_not_reached"))
        self._emergency_status = str(validation.get("status_emergency", "contact_reachability_emergency_stop"))

        self._contact_validation_min_force = float(contact_gate.get("contact_validation_min_force_n", 0.02))
        self._target_name = str(target.get("target_name", "proposal_v2_8_contact_reachability_pad"))
        self._target_type = str(target.get("contact_target_type", target.get("target_type", "reachable_runtime_spawned_calibration_pad")))
        self._target_contact_topic = str(topic_wiring.get("raw_contact_topic", target.get("contact_topic", "/proposal_simulation_cell/contact_reachability_contacts")))
        self._target_gz_contact_topic = str(target.get("gz_contact_topic", ""))
        self._target_frame = str(target.get("frame_id", "contact_reachability_pad"))
        ...
        self._status_pub = self.create_publisher(
            String,
            str(validation.get("status_topic", "/proposal_simulation_cell/contact_reachability_status")),
            10,
        )
        ...
        self._safety_pub = self.create_publisher(
            String, str(validation.get("safety_report_topic", "/proposal_simulation_cell/contact_reachability_safety_report")), 10
        )
        self._endpoint_pub = self.create_publisher(
            String, str(validation.get("endpoint_report_topic", "/proposal_simulation_cell/contact_reachability_endpoint_report")), 10
        )
        ...
        self.get_logger().info("proposal_simulation_cell_v2_8 contact reachability trigger node started")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"proposal v2.8 config not found: {path}")
    ...
    def _on_contacts(self, message: Any) -> None:
        ...
        force, torque, contact_count, max_depth = self._extract_contact_wrench(message)
        self._raw_contact_count += contact_count
        self._max_contact_depth = max(self._max_contact_depth, max_depth)
        if force <= 0.0 and torque <= 0.0 and max_depth <= 0.0:
            return
    ...
    def _tick(self) -> None:
        ...
        if elapsed >= self._validation_timeout and not self._finished:
            self._write_outputs_once("contact_reachability_timeout")
    ...
    def _run_sequence(self) -> None:
        ...
        if not self._target_spawned:
            ...
            return
        self._record_phase(
            "compute_reachable_contact_pad_pose",
            "succeeded" if self._contact_pad_pose_computed else "failed",
            "Calibration pad pose computed from distal collision frame path",
            0.0,
            0.0,
        )
        self._record_phase(
            "spawn_or_reposition_contact_pad_on_tool_path",
            "succeeded" if self._target_spawned and self._contact_pad_on_touch_path else "failed",
            "Runtime calibration pad placed on guarded touch path",
            0.0,
            0.0,
        )
        self._wait_for_settle(1.0)
        self._execute_guarded_touch_steps()
        self._snapshots["after_contact_trigger_steps"] = self._last_joint_state
        self._record_phase(
    ...
    def _execute_guarded_touch_steps(self) -> None:
        ...
        self._record_phase("execute_contact_trigger_steps", "succeeded", "Contact trigger step loop started", 0.0, 0.0)
    ...
    def _compute_and_spawn_contact_target_on_path(self) -> None:
        ...
        self._target_spawn_pose = [base_pose[i] + self._target_offset[i] for i in range(3)]
    ...
    def _status_payload(self, status: str | None = None) -> dict[str, Any]:
        return {
            ...
            "contact_pad_pose_computed": self._contact_pad_pose_computed,
            "contact_pad_spawned_or_repositioned": self._target_spawned,
            "contact_pad_reachable": self._contact_pad_reachable,
            "contact_pad_on_touch_path": self._contact_pad_on_touch_path,
            "contact_collision_enabled": self._contact_collision_enabled,
            ...
            "stop_on_contact_executed": self._contact_gate_triggered,
            ...
            "status": status or "contact_reachability_pending",
        }
    ...
    def _write_outputs_once(self, status: str) -> None:
        ...
        for file_name, snapshot in [
            ...
            ("joint_states_after_contact_trigger_steps.txt", "after_contact_trigger_steps"),
        ...
        self._write_csv(self._output_dir / "contact_trigger_step_report.csv", self._step_rows)
        self._write_csv(self._output_dir / "contact_trigger_validation_report.csv", self._contact_rows)
        self._write_csv(self._output_dir / "contact_reachability_safety_report.csv", self._safety_rows())
        self._write_csv(self._output_dir / "contact_reachability_endpoint_report.csv", self._endpoint_rows)
        self._write_lines(self._output_dir / "contact_pad_pose_report.txt", self._target_report_lines)
        self._write_lines(self._output_dir / "contact_topic_wiring_report.txt", self._topic_wiring_lines())
        self._write_json(self._output_dir / "contact_reachability_and_trigger_status.json", payload)
        self._write_summary(payload)
        self._write_run_log(payload)
        self._publish_reports()
        self.get_logger().info("proposal_simulation_cell_v2_8 diagnostics written")
        rclpy.shutdown()
    ...
    def _write_summary(self, status: dict[str, Any]) -> None:
        lines = [
            ...
            "",
            "This diagnostic validates a reachable simulation contact pad, raw contact topic wiring, derived contact force, stop-on-contact, retreat, and return-to-ready behavior.",
            "",
            f"- tool_pose_available: {self._bool(status['tool_pose_available'])}",
            f"- contact_pad_pose_computed: {self._bool(status['contact_pad_pose_computed'])}",
            f"- contact_pad_on_touch_path: {self._bool(status['contact_pad_on_touch_path'])}",
            f"- raw_contact_topic_available: {self._bool(status['raw_contact_topic_available'])}",
    ...
    def _extract_contact_wrench(self, message: Any) -> tuple[float, float, int, float]:
        max_force = 0.0
        max_torque = 0.0
        max_depth = 0.0
        ...
        return max_force, max_torque, len(contacts), max_depth
...
def main(args: list[str] | None = None) -> None:
    ...
    node = ProposalSimulationCellV28ContactReachabilityTriggerNode()
    try:
        rclpy.spin(node)
    finally:
        if rclpy.ok():
