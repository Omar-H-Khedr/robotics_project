"""Contact-gated guarded approach validation for proposal_simulation_cell_v2_6."""
...
class ProposalSimulationCellV26ContactGatedGuardedApproachNode(Node):
    """Execute a bounded guarded approach through the verified Gazebo controller."""

    def __init__(self) -> None:
        super().__init__("proposal_simulation_cell_v2_6_contact_gated_guarded_approach_node")
        self.declare_parameter("config_path", "")
        self.declare_parameter("output_dir", "diagnostics/proposal_simulation_cell_v2_6")

        self._config = self._load_config()
        robot = self._config.get("robot", {})
        task = self._config.get("task_sequence", {})
        moveit = self._config.get("moveit_execution", {})
        approach = self._config.get("guarded_approach", {})
        ...
        self._output_dir = Path(
            ...
            or validation.get("output_dir", "diagnostics/proposal_simulation_cell_v2_6")
        ).expanduser()
        self._output_dir.mkdir(parents=True, exist_ok=True)

        self._simulation_engine = str(task.get("simulation_engine", "gazebo"))
        self._task_sequence_type = str(task.get("task_sequence_type", "contact_gated_guarded_approach"))
        self._phase_names = [str(name) for name in task.get("phases", [])]
        self._max_task_duration = float(task.get("max_task_duration_sec", 60.0))
        ...
        self._velocity_scale = float(moveit.get("max_velocity_scaling_factor", 0.08))
        self._acceleration_scale = float(moveit.get("max_acceleration_scaling_factor", 0.08))
        self._joint_goal_tolerance = float(moveit.get("joint_goal_tolerance_rad", 0.012))
        self._max_joint_delta_deg = float(approach.get("max_joint_delta_from_initial_deg", 7.0))
        self._step_count = int(approach.get("guarded_approach_step_count", 5))
        self._max_step_delta_deg = float(approach.get("max_single_step_joint_delta_deg", 1.0))
        self._ready_delta = self._delta_map(approach.get("ready_pose_delta_deg", {}))
        self._pre_approach_delta = self._delta_map(approach.get("pre_approach_pose_delta_deg", {}))
        self._standby_delta = self._delta_map(approach.get("pre_contact_standby_pose_delta_deg", {}))
        self._guarded_step_delta = self._delta_map(approach.get("guarded_step_delta_deg", {}))
        ...
        self._validation_timeout = float(validation.get("validation_timeout_sec", 180.0))
        self._startup_wait = float(validation.get("startup_wait_sec", 5.0))
        self._success_contact_status = str(validation.get("status_success_contact", "contact_gated_guarded_approach_validated"))
        self._success_no_contact_status = str(
            validation.get("status_success_no_contact", "contact_gated_guarded_approach_validated_no_contact_detected")
        )

        self._status_pub = self.create_publisher(
            String,
            str(validation.get("status_topic", "/proposal_simulation_cell/contact_gated_guarded_approach_status")),
            10,
        )
        self._step_pub = self.create_publisher(
            String,
            str(validation.get("step_report_topic", "/proposal_simulation_cell/guarded_approach_step_report")),
            10,
        )
        self._contact_gate_pub = self.create_publisher(
            String, str(validation.get("contact_gate_report_topic", "/proposal_simulation_cell/contact_gate_report")), 10
        )
        self._safety_pub = self.create_publisher(
            String, str(validation.get("safety_report_topic", "/proposal_simulation_cell/contact_gated_safety_report")), 10
        )
        self._endpoint_pub = self.create_publisher(
            String, str(validation.get("endpoint_report_topic", "/proposal_simulation_cell/contact_gated_endpoint_report")), 10
        )
        ...
        self._contact_gate_rows: list[dict[str, str]] = []
        ...
        self.get_logger().info("proposal_simulation_cell_v2_6 contact-gated guarded approach node started")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"proposal v2.6 config not found: {path}")
    ...
    def _tick(self) -> None:
        ...
        if elapsed >= self._validation_timeout and not self._finished:
            self._write_outputs_once("contact_gated_guarded_approach_timeout")
    ...
    def _run_sequence(self) -> None:
        ...
        phases = [
            ...
            ("move_to_pre_contact_standby_pose", "after_pre_contact_standby_pose", self._standby_positions),
        ]
        for phase_name, snapshot_name, target in phases:
            ...
            if not self._execute_motion_phase(phase_name, snapshot_name, target):
                self._write_outputs_once("contact_gated_guarded_approach_execution_failed")
                return

        if not self._execute_guarded_approach_steps():
            if self._emergency_stop_triggered:
                self._retreat_if_possible()
                self._write_outputs_once("guarded_approach_emergency_stop_triggered")
                return
            self._write_outputs_once("contact_gated_guarded_approach_execution_failed")
            return
        self._snapshots["after_guarded_approach"] = self._last_joint_state
        self._record_phase(
            "stop_on_contact_or_standoff_limit",
            "succeeded",
            "Guarded approach stopped by contact gate or stand-off step limit",
            0.0,
            0.0,
        )
        if not self._execute_motion_phase("retreat_to_pre_contact_standby_pose", "after_retreat", self._standby_positions):
            self._write_outputs_once("contact_gated_guarded_approach_execution_failed")
            return
        self._retreat_completed = True
        ...
        if self._emergency_stop_triggered:
            self._write_outputs_once("guarded_approach_emergency_stop_triggered")
            return
        self._write_outputs_once(self._success_contact_status if self._contact_gate_triggered else self._success_no_contact_status)
    ...
    def _execute_motion_phase(self, phase_name: str, snapshot_name: str, target: dict[str, float]) -> bool:
        ...
        if phase_name == "move_to_pre_contact_standby_pose":
    ...
    def _execute_guarded_approach_steps(self) -> bool:
        self._guarded_approach_started = True
        self._record_phase("execute_guarded_approach_steps", "succeeded", "Guarded approach step loop started", 0.0, 0.0)
        ...
        for index in range(1, self._step_count + 1):
            if not self._verify_endpoint():
                self._record_guarded_step(index, "failed", "Gazebo endpoint lost", current_target, 0.0)
                return False
            if self._safety_blocked():
                self._record_guarded_step(index, "failed", "Safety gate blocked", current_target, 0.0)
                return False
            ...
            if not self._single_step_within_limit(current_target, next_target) or not self._target_within_limits(next_target):
                self._record_guarded_step(index, "failed", "Guarded step exceeds bounded joint delta", next_target, 0.0)
                return False
            self._guarded_steps_attempted += 1
            start = time.monotonic()
            executed = self._execute_trajectory(self._direct_joint_trajectory(next_target, duration_sec=1.2))
            duration = time.monotonic() - start
            ...
            self._record_guarded_step(index, "succeeded" if step_observed else "failed", "Guarded step executed", current, duration)
            self._record_contact_gate(index)
            if self._emergency_stop_triggered:
                self._record_phase("execute_guarded_approach_steps", "failed", "Emergency stop threshold exceeded", duration, 0.0)
                return False
            if self._last_force >= self._contact_threshold:
                self._contact_gate_triggered = True
                return True
            if not step_observed:
                return False
        self._standoff_limit_reached = True
        self._record_contact_gate(self._step_count)
        return True

    def _retreat_if_possible(self) -> None:
        if self._standby_positions and self._endpoint_verified:
            self._execute_trajectory(self._direct_joint_trajectory(self._standby_positions, duration_sec=1.5))
            self._snapshots["after_retreat"] = self._last_joint_state
            self._retreat_completed = True
    ...
    def _safety_blocked(self) -> bool:
        return (
            self._safety_violation_count > 0
            or self._emergency_stop_triggered
            or self._peg_insertion_allowed
            or self._forceful_contact_allowed
        )
            ...
            self._plan_rows.append(
                {
                    "phase": phase_name,
                    "plan_found": "false",
                    "plan_error_code": "joint_delta_limit",
                    "trajectory_point_count": "0",
                    "trajectory_joint_count": "0",
                    "planned_duration_sec": "0.000000",
                }
            )
            return False
        return True
    ...
    def _record_guarded_step(self, step_index: int, status: str, note: str, positions: dict[str, float], duration: float) -> None:
        max_delta = max(
            abs(math.degrees(positions.get(joint, 0.0) - self._initial_positions.get(joint, 0.0)))
            for joint in self._joint_names
        )
        self._step_rows.append(
            {
                ...
                "status": status,
                "duration_sec": f"{duration:.6f}",
                "max_delta_from_initial_deg": f"{max_delta:.6f}",
                "force_n": f"{self._last_force:.6f}",
                "torque_nm": f"{self._last_torque:.6f}",
                "contact_gate_triggered": self._bool(self._last_force >= self._contact_threshold),
                "emergency_stop_triggered": self._bool(self._emergency_stop_triggered),
                "note": note,
            }
        )

    def _record_contact_gate(self, step_index: int) -> None:
        self._contact_gate_rows.append(
            {
                "step_index": str(step_index),
                "force_n": f"{self._last_force:.6f}",
                "torque_nm": f"{self._last_torque:.6f}",
                ...
                "contact_gate_triggered": self._bool(self._last_force >= self._contact_threshold),
                "standoff_limit_reached": self._bool(self._standoff_limit_reached),
                "emergency_stop_triggered": self._bool(self._emergency_stop_triggered),
            }
        )
    ...
    def _write_joint_rows(self) -> None:
        mapping = [
            ...
            ("pre_contact_standby", "after_pre_contact_standby_pose"),
            ("guarded_approach", "after_guarded_approach"),
    ...
    def _status_payload(self, status: str | None = None) -> dict[str, Any]:
        return {
            ...
            "pre_contact_standby_pose_reached": self._standby_pose_reached,
            "guarded_approach_started": self._guarded_approach_started,
            "guarded_approach_steps_attempted": self._guarded_steps_attempted,
            "guarded_approach_steps_completed": self._guarded_steps_completed,
            ...
            "standoff_limit_reached": self._standoff_limit_reached,
            ...
            "status": status or "contact_gated_guarded_approach_pending",
        }
    ...
    def _publish_reports(self) -> None:
        ...
        self._publish_json(self._contact_gate_pub, {"rows": self._contact_gate_rows})
    ...
    def _write_outputs_once(self, status: str) -> None:
        ...
        self._write_lines(
            self._output_dir / "joint_states_after_pre_contact_standby_pose.txt",
            self._joint_state_lines(self._snapshots.get("after_pre_contact_standby_pose")),
        )
        self._write_lines(
            self._output_dir / "joint_states_after_guarded_approach.txt",
            self._joint_state_lines(self._snapshots.get("after_guarded_approach")),
        )
        self._write_lines(self._output_dir / "joint_states_after_retreat.txt", self._joint_state_lines(self._snapshots.get("after_retreat")))
        ...
        self._write_csv(self._output_dir / "guarded_approach_step_report.csv", self._step_rows)
        self._write_csv(self._output_dir / "contact_gate_report.csv", self._contact_gate_rows)
        self._write_csv(self._output_dir / "contact_gated_safety_report.csv", self._safety_rows())
        self._write_csv(self._output_dir / "contact_gated_endpoint_report.csv", self._endpoint_rows)
        self._write_json(self._output_dir / "contact_gated_guarded_approach_status.json", payload)
        self._write_summary(payload)
        self._write_run_log(payload)
        self._publish_reports()
        self.get_logger().info("proposal_simulation_cell_v2_6 diagnostics written")
        rclpy.shutdown()
    ...
    def _write_summary(self, status: dict[str, Any]) -> None:
        lines = [
            ...
            "",
            "This diagnostic executes a contact-gated guarded approach through the verified Gazebo simulation controller.",
            "",
            f"- phases_planned: {status['phases_planned']}",
            f"- phases_executed: {status['phases_executed']}",
            f"- guarded_approach_steps_completed: {status['guarded_approach_steps_completed']}",
            f"- contact_gate_triggered: {self._bool(status['contact_gate_triggered'])}",
            f"- standoff_limit_reached: {self._bool(status['standoff_limit_reached'])}",
    ...
    def _write_run_log(self, status: dict[str, Any]) -> None:
        lines = [
            "proposal_simulation_cell_v2_6_contact_gated_guarded_approach_validation",
            f"status={status['status']}",
            f"all_required_phase_plans_found={self._bool(status['all_required_phase_plans_found'])}",
            f"all_required_phase_executions_observed={self._bool(status['all_required_phase_executions_observed'])}",
            f"guarded_approach_started={self._bool(status['guarded_approach_started'])}",
            f"contact_gate_triggered={self._bool(status['contact_gate_triggered'])}",
            f"standoff_limit_reached={self._bool(status['standoff_limit_reached'])}",
            f"retreat_completed={self._bool(status['retreat_completed'])}",
            f"return_to_ready_completed={self._bool(status['return_to_ready_completed'])}",
...
def main(args: list[str] | None = None) -> None:
    ...
    node = ProposalSimulationCellV26ContactGatedGuardedApproachNode()
    try:
        rclpy.spin(node)
    finally:
        if rclpy.ok():
