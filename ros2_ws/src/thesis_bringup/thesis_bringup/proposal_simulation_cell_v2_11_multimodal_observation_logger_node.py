"""Multimodal contact observation logging for proposal_simulation_cell_v2_11."""
...
class ProposalSimulationCellV211MultimodalObservationLoggerNode(Node):
    """Run the validated Gazebo-only contact batch and log synchronized observations."""

    def __init__(self) -> None:
        super().__init__("proposal_simulation_cell_v2_11_multimodal_observation_logger_node")
        self.declare_parameter("config_path", "")
        self.declare_parameter("output_dir", "diagnostics/proposal_simulation_cell_v2_11")
        ...
        multimodal = self._config.get("multimodal_topics", {})
        observation = self._config.get("observation_logging", {})
        ...
        self._output_dir = Path(
            ...
            or validation.get("output_dir", "diagnostics/proposal_simulation_cell_v2_11")
        ).expanduser()
        self._output_dir.mkdir(parents=True, exist_ok=True)

        self._simulation_engine = str(task.get("simulation_engine", "gazebo"))
        self._task_sequence_type = str(task.get("task_sequence_type", "multimodal_contact_observation_logging"))
        ...
        self._success_status = str(validation.get("status_success", "multimodal_contact_observation_logging_validated"))
        self._partial_status = str(validation.get("status_partial", "multimodal_contact_observation_logging_partially_validated"))
        self._rgbd_unavailable_status = str(
            validation.get("status_rgbd_unavailable", "contact_observation_logging_validated_rgbd_unavailable")
        )
        self._no_contact_status = str(validation.get("status_no_contact", "misalignment_batch_contact_not_triggered"))
        self._emergency_status = str(validation.get("status_safety_limit", "misalignment_batch_safety_limit_exceeded"))
        self._geometry_status = str(validation.get("status_geometry_unavailable", "misalignment_batch_geometry_unavailable"))
        self._ik_status = str(validation.get("status_ik_failed", "misalignment_batch_ik_reachability_failed"))
        ...
        self._scenario_count_required = int(task.get("scenario_count", len(self._scenarios)))
        self._rgb_topic = str(multimodal.get("rgb_topic", "/proposal_simulation_cell/d405/color/image_raw"))
        self._depth_topic = str(multimodal.get("depth_topic", "/proposal_simulation_cell/d405/depth/image_rect_raw"))
        self._rgb_info_topic = str(multimodal.get("color_camera_info_topic", "/proposal_simulation_cell/d405/color/camera_info"))
        self._depth_info_topic = str(multimodal.get("depth_camera_info_topic", "/proposal_simulation_cell/d405/depth/camera_info"))
        self._write_full_image_data = bool(observation.get("write_full_image_data", False))
        self._fake_dataset_created = bool(observation.get("fake_dataset_created", False))
        self._experimental_result_created = bool(observation.get("experimental_result_created", False))
        self._target_name = str(target.get("target_name", "proposal_v2_11_multimodal_contact_pad"))
        self._target_type = str(target.get("contact_target_type", target.get("target_type", "computed_non_overlapping_multimodal_contact_pad")))
        self._target_contact_topic = str(topic_wiring.get("raw_contact_topic", target.get("contact_topic", "/proposal_simulation_cell/multimodal_contact_contacts")))
        self._target_gz_contact_topic = str(target.get("gz_contact_topic", ""))
        self._target_frame = str(target.get("frame_id", "multimodal_contact_pad"))
        ...
        self._observation_rows: list[dict[str, str]] = []
        self._contact_transition_observation_rows: list[dict[str, str]] = []
        self._rgbd_frame_rows: list[dict[str, str]] = []
        self._rgb_frame_count = 0
        self._depth_frame_count = 0
        self._rgb_info_count = 0
        self._depth_info_count = 0
        self._last_rgb_image: Image | None = None
        self._last_depth_image: Image | None = None
        self._last_rgb_info: CameraInfo | None = None
        self._last_depth_info: CameraInfo | None = None
        self._scenario_rgb_start = 0
        self._scenario_depth_start = 0

        self._status_pub = self.create_publisher(
            String,
            str(validation.get("status_topic", "/proposal_simulation_cell/multimodal_contact_observation_status")),
            10,
        )
        self._step_pub = self.create_publisher(
            String, str(validation.get("scenario_report_topic", "/proposal_simulation_cell/multimodal_scenario_observation_report")), 10
        )
        self._contact_pub = self.create_publisher(
            String,
            str(validation.get("contact_transition_report_topic", "/proposal_simulation_cell/contact_transition_observation_report")),
            10,
        )
        ...
        self._safety_pub = self.create_publisher(
            String, str(validation.get("safety_report_topic", "/proposal_simulation_cell/multimodal_contact_safety_report")), 10
        )
        ...
        self.get_logger().info("proposal_simulation_cell_v2_11 multimodal observation logger node started")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"proposal v2.11 config not found: {path}")
    ...
    def _tick(self) -> None:
        ...
        if elapsed >= self._validation_timeout and not self._finished:
            self._write_outputs_once("multimodal_contact_observation_timeout")

    def _run_sequence(self) -> None:
        self._record_phase("initialize", "succeeded", "batch sequence initialized", 0.0, 0.0)
        ...
        elif attempted >= self._scenario_count_required and validated >= self._scenario_count_required and self._all_batch_requirements_met():
            status = self._success_status if self._rgbd_available_for_full_validation() else self._rgbd_unavailable_status
            self._write_outputs_once(status)
        elif validated >= 3:
    ...
    def _run_scenario(self, scenario: dict[str, Any]) -> None:
        ...
        self._scenario_rgb_start = self._rgb_frame_count
        self._scenario_depth_start = self._depth_frame_count
        self._reset_scenario_state()
        self._record_phase(self._phase_name("start"), "succeeded", "Scenario started", 0.0, 0.0)
    ...
    def _record_phase(self, phase: str, status: str, note: str, duration: float, final_error: float) -> None:
        ...
        self._record_observation_row(phase, 0)
    ...
    def _record_guarded_step(self, step_index: int, status: str, positions: dict[str, float], duration: float) -> None:
        ...
        self._record_observation_row("execute_contact_trigger_steps", step_index)
    ...
    def _record_contact_trigger(self, step_index: int) -> None:
        ...
        transition_row = dict(row)
        transition_row.update(
            {
                "timestamp_sec": f"{time.monotonic() - self._start_time:.6f}",
                "phase_name": "contact_transition",
                "rgb_frame_count": str(self._rgb_frame_count),
                "depth_frame_count": str(self._depth_frame_count),
                "initial_no_contact_verified": self._bool(self._initial_no_contact_verified),
                "stop_on_contact_executed": self._bool(self._contact_gate_triggered),
                "retreat_completed": self._bool(self._retreat_completed),
            }
        )
        self._contact_transition_observation_rows.append(transition_row)
    ...
    def _record_observation_row(self, phase_name: str, step_index: int) -> None:
        if not self._current_scenario_name:
            return
        joint_positions = self._joint_positions(self._last_joint_state)
        velocities = self._joint_velocities(self._last_joint_state)
        contact_pose = self._lookup_frame_pose(self._target_collision_frame) or [0.0, 0.0, 0.0]
        tool_pose = self._lookup_frame_pose(self._target_tool_frame) or [0.0, 0.0, 0.0]
        scenario = self._current_scenario
        self._observation_rows.append(
            {
                "timestamp_sec": f"{time.monotonic() - self._start_time:.6f}",
                "scenario_id": self._current_scenario_name,
                "phase_name": phase_name,
                "step_index": str(step_index),
                "x_offset_m": f"{float(scenario.get('x_offset_m', 0.0)):.6f}",
                "y_offset_m": f"{float(scenario.get('y_offset_m', 0.0)):.6f}",
                "yaw_offset_deg": f"{float(scenario.get('yaw_offset_deg', 0.0)):.6f}",
                "joint_positions": self._position_summary(joint_positions),
                "joint_velocities": self._velocity_summary(velocities),
                "selected_contact_link_pose_xyz": self._xyz_summary(contact_pose),
                "tool0_pose_xyz": self._xyz_summary(tool_pose),
                "contact_force_n": f"{self._last_force:.6f}",
                "contact_torque_nm": f"{self._last_torque:.6f}",
                "contact_gate_triggered": self._bool(self._contact_gate_triggered),
                "initial_no_contact_verified": self._bool(self._initial_no_contact_verified),
                "stop_on_contact_executed": self._bool(self._contact_gate_triggered),
                "retreat_completed": self._bool(self._retreat_completed),
                "rgb_topic_available": self._bool(self._rgb_topic_available()),
                "depth_topic_available": self._bool(self._depth_topic_available()),
                "camera_info_available": self._bool(self._camera_info_available()),
                "rgb_frame_count": str(self._rgb_frame_count),
                "depth_frame_count": str(self._depth_frame_count),
            }
        )

    def _record_rgbd_frame_row(self) -> None:
        self._rgbd_frame_rows.append(
            {
                "scenario": self._current_scenario_name,
                "rgb_topic_available": self._bool(self._rgb_topic_available()),
                "depth_topic_available": self._bool(self._depth_topic_available()),
                "camera_info_available": self._bool(self._camera_info_available()),
                "rgb_frame_count_start": str(self._scenario_rgb_start),
                "rgb_frame_count_end": str(self._rgb_frame_count),
                "rgb_frame_count_delta": str(max(0, self._rgb_frame_count - self._scenario_rgb_start)),
                "depth_frame_count_start": str(self._scenario_depth_start),
                "depth_frame_count_end": str(self._depth_frame_count),
                "depth_frame_count_delta": str(max(0, self._depth_frame_count - self._scenario_depth_start)),
                "rgb_encoding": str(getattr(self._last_rgb_image, "encoding", "")) if self._last_rgb_image else "",
                "depth_encoding": str(getattr(self._last_depth_image, "encoding", "")) if self._last_depth_image else "",
            }
        )
    ...
    def _status_payload(self, status: str | None = None) -> dict[str, Any]:
        ...
        attempted = len(self._scenario_results)
        validated = sum(1 for result in self._scenario_results if result.get("validated"))
        failed = [str(result.get("name")) for result in self._scenario_results if not result.get("validated")]
        ...
        by_name = {str(result.get("name")): bool(result.get("validated")) for result in self._scenario_results}
        return {
            ...
            "scenarios_completed": attempted,
            "scenarios_validated": validated,
            "rgb_topic_available": self._rgb_topic_available(),
            "depth_topic_available": self._depth_topic_available(),
            "camera_info_available": self._camera_info_available(),
            "joint_states_available": self._last_joint_state is not None,
            "tf_available": self._tool_pose_available,
            "contact_wrench_available": self._contact_wrench_topic_available or self._contact_wrench_topic in self._topic_names(),
            "raw_contact_topic_available": self._raw_contact_topic_available or self._target_contact_topic in self._topic_names(),
            "observation_log_written": bool(self._observation_rows),
            "observation_row_count": len(self._observation_rows),
            "contact_transition_log_written": bool(self._contact_transition_observation_rows),
            "contact_transition_row_count": len(self._contact_transition_observation_rows),
            "rgb_frame_count_total": self._rgb_frame_count,
            "depth_frame_count_total": self._depth_frame_count,
            "scenario_success_rate": float(validated) / float(scenario_count) if scenario_count else 0.0,
            "all_initial_no_contact_verified": bool(attempted and all(r.get("initial_no_contact_verified") for r in self._scenario_results)),
            "all_scenarios_initial_no_contact_verified": bool(attempted and all(r.get("initial_no_contact_verified") for r in self._scenario_results)),
            ...
            "nominal_centered_validated": by_name.get("nominal_centered", False),
            "x_positive_offset_validated": by_name.get("x_positive_offset", False),
            "x_negative_offset_validated": by_name.get("x_negative_offset", False),
            "y_positive_offset_validated": by_name.get("y_positive_offset", False),
            "y_negative_offset_validated": by_name.get("y_negative_offset", False),
            "fake_dataset_created": self._fake_dataset_created,
            "experimental_result_created": self._experimental_result_created,
            ...
            "status": status or "multimodal_contact_observation_pending",
        }
    ...
    def _write_outputs_once(self, status: str) -> None:
        ...
        self._write_json(self._output_dir / "multimodal_topic_status.json", self._multimodal_topic_status())
        self._write_csv(self._output_dir / "scenario_definition_table.csv", self._scenario_definition_rows)
        self._write_csv(self._output_dir / "scenario_execution_summary.csv", self._scenario_execution_summary_rows())
        self._write_csv(self._output_dir / "multimodal_observation_log.csv", self._observation_rows)
        self._write_csv(self._output_dir / "contact_transition_observation_log.csv", self._contact_transition_observation_rows)
        self._write_csv(self._output_dir / "rgbd_frame_count_report.csv", self._rgbd_frame_rows)
        self._write_csv(self._output_dir / "observation_channel_completeness_report.csv", self._observation_channel_rows())
        self._write_csv(self._output_dir / "scenario_safety_report.csv", self._safety_rows())
        self._write_csv(self._output_dir / "scenario_endpoint_report.csv", self._endpoint_rows)
        self._write_json(self._output_dir / "multimodal_contact_observation_status.json", payload)
        self._write_summary(payload)
        self._write_run_log(payload)
        self._publish_reports()
        self.get_logger().info("proposal_simulation_cell_v2_11 diagnostics written")
        rclpy.shutdown()
    ...
    def _multimodal_topic_status(self) -> dict[str, Any]:
        return {
            "rgb_topic": self._rgb_topic,
            "depth_topic": self._depth_topic,
            "color_camera_info_topic": self._rgb_info_topic,
            "depth_camera_info_topic": self._depth_info_topic,
            "contact_wrench_topic": self._contact_wrench_topic,
            "raw_contact_topic": self._target_contact_topic,
            "rgb_topic_available": self._rgb_topic_available(),
            "depth_topic_available": self._depth_topic_available(),
            "camera_info_available": self._camera_info_available(),
            "joint_states_available": self._last_joint_state is not None,
            ...
            "rgb_frame_count_total": self._rgb_frame_count,
            "depth_frame_count_total": self._depth_frame_count,
            "rgb_camera_info_count": self._rgb_info_count,
            "depth_camera_info_count": self._depth_info_count,
            "write_full_image_data": self._write_full_image_data,
            "fake_dataset_created": self._fake_dataset_created,
            "experimental_result_created": self._experimental_result_created,
        }

    def _scenario_execution_summary_rows(self) -> list[dict[str, str]]:
        return [
            {
                "scenario": str(result.get("name", "")),
                "validated": self._bool(result.get("validated")),
                "initial_no_contact_verified": self._bool(result.get("initial_no_contact_verified")),
                "contact_trigger_after_motion": self._bool(result.get("contact_trigger_after_motion")),
                "contact_trigger_step_index": str(result.get("contact_trigger_step_index", "")),
                "stop_on_contact_executed": self._bool(result.get("stop_on_contact_executed")),
                "retreat_completed": self._bool(result.get("retreat_completed")),
                "post_retreat_no_contact_verified": self._bool(result.get("post_retreat_no_contact_verified")),
                "return_to_ready_completed": self._bool(result.get("return_to_ready_completed")),
    ...
    def _observation_channel_rows(self) -> list[dict[str, str]]:
        payload = self._multimodal_topic_status()
        return [
            {"channel": "rgb_image", "available": self._bool(payload["rgb_topic_available"]), "sample_count": str(self._rgb_frame_count)},
            {"channel": "depth_image", "available": self._bool(payload["depth_topic_available"]), "sample_count": str(self._depth_frame_count)},
            {"channel": "camera_info", "available": self._bool(payload["camera_info_available"]), "sample_count": str(self._rgb_info_count + self._depth_info_count)},
            {"channel": "joint_states", "available": self._bool(payload["joint_states_available"]), "sample_count": str(len(self._observation_rows))},
            {"channel": "tf_tool_pose", "available": self._bool(payload["tf_available"]), "sample_count": str(len(self._observation_rows))},
            {"channel": "contact_wrench", "available": self._bool(payload["contact_wrench_available"]), "sample_count": str(len(self._contact_transition_observation_rows))},
            {"channel": "raw_contact", "available": self._bool(payload["raw_contact_topic_available"]), "sample_count": str(self._raw_contact_message_count)},
            {"channel": "task_phase", "available": self._bool(bool(self._phase_rows)), "sample_count": str(len(self._phase_rows))},
            {"channel": "scenario_metadata", "available": self._bool(bool(self._scenario_definition_rows)), "sample_count": str(len(self._scenario_definition_rows))},
            {"channel": "observation_vector", "available": self._bool(bool(self._observation_rows)), "sample_count": str(len(self._observation_rows))},
            {"channel": "contact_transition_label", "available": self._bool(bool(self._contact_transition_observation_rows)), "sample_count": str(len(self._contact_transition_observation_rows))},
        ]
    ...
    def _write_summary(self, status: dict[str, Any]) -> None:
        lines = [
            ...
            "",
            "This diagnostic replays the validated Gazebo-only contact-gate batch and records synchronized multimodal observation rows for later context-inference work.",
            "",
            f"- scenario_count: {status['scenario_count']}",
            f"- scenarios_attempted: {status['scenarios_attempted']}",
            f"- scenarios_validated: {status['scenarios_validated']}",
            f"- rgb_topic_available: {self._bool(status['rgb_topic_available'])}",
            f"- depth_topic_available: {self._bool(status['depth_topic_available'])}",
            f"- camera_info_available: {self._bool(status['camera_info_available'])}",
            f"- observation_row_count: {status['observation_row_count']}",
            f"- contact_transition_row_count: {status['contact_transition_row_count']}",
    ...
    def _joint_velocities(self, message: JointState | None) -> dict[str, float]:
        if message is None:
            return {}
        return {name: float(velocity) for name, velocity in zip(message.name, message.velocity)}
    ...
    def _velocity_summary(self, velocities: dict[str, float]) -> str:
        if not velocities:
            return "unavailable"
        return ";".join(f"{joint}={velocities.get(joint, 0.0):.9f}" for joint in self._joint_names)

    def _xyz_summary(self, xyz: list[float]) -> str:
        return ",".join(f"{float(value):.6f}" for value in xyz[:3])
    ...
    def _rgb_topic_available(self) -> bool:
        return self._rgb_frame_count > 0 or self._rgb_topic in self._topic_names()

    def _depth_topic_available(self) -> bool:
        return self._depth_frame_count > 0 or self._depth_topic in self._topic_names()

    def _camera_info_available(self) -> bool:
        topics = self._topic_names()
        return self._rgb_info_count > 0 or self._depth_info_count > 0 or self._rgb_info_topic in topics or self._depth_info_topic in topics

    def _rgbd_available_for_full_validation(self) -> bool:
        return self._rgb_topic_available() and self._depth_topic_available()
...
def main(args: list[str] | None = None) -> None:
    ...
    node = ProposalSimulationCellV211MultimodalObservationLoggerNode()
    try:
        rclpy.spin(node)
    finally:
        if rclpy.ok():
