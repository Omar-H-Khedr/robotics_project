"""Misalignment contact-gate batch validation for proposal_simulation_cell_v2_10."""
...
class ProposalSimulationCellV210MisalignmentContactBatchNode(Node):
    """Run a bounded Gazebo-only no-contact standby then contact approach."""

    def __init__(self) -> None:
        super().__init__("proposal_simulation_cell_v2_10_misalignment_contact_batch_node")
        self.declare_parameter("config_path", "")
        self.declare_parameter("output_dir", "diagnostics/proposal_simulation_cell_v2_10")
        ...
        self._output_dir = Path(
            ...
            or validation.get("output_dir", "diagnostics/proposal_simulation_cell_v2_10")
        ).expanduser()
        self._output_dir.mkdir(parents=True, exist_ok=True)

        self._simulation_engine = str(task.get("simulation_engine", "gazebo"))
        self._task_sequence_type = str(task.get("task_sequence_type", "misalignment_contact_gate_batch_validation"))
        ...
        self._success_status = str(validation.get("status_success", "misalignment_contact_gate_batch_validated"))
        self._partial_status = str(validation.get("status_partial", "misalignment_contact_gate_batch_partially_validated"))
        ...
        self._target_name = str(target.get("target_name", "proposal_v2_10_misalignment_contact_pad"))
        self._target_type = str(target.get("contact_target_type", target.get("target_type", "computed_non_overlapping_misalignment_contact_pad")))
        self._target_contact_topic = str(topic_wiring.get("raw_contact_topic", target.get("contact_topic", "/proposal_simulation_cell/misalignment_contact_contacts")))
        self._target_gz_contact_topic = str(target.get("gz_contact_topic", ""))
        self._target_frame = str(target.get("frame_id", "misalignment_contact_pad"))
        ...
        self._status_pub = self.create_publisher(
            String,
            str(validation.get("status_topic", "/proposal_simulation_cell/misalignment_contact_batch_status")),
            10,
        )
        self._step_pub = self.create_publisher(
            String, str(validation.get("scenario_report_topic", "/proposal_simulation_cell/misalignment_scenario_report")), 10
        )
        self._contact_pub = self.create_publisher(
            String,
            str(validation.get("contact_transition_report_topic", "/proposal_simulation_cell/misalignment_contact_transition_report")),
            10,
        )
        ...
        self._safety_pub = self.create_publisher(
            String, str(validation.get("safety_report_topic", "/proposal_simulation_cell/misalignment_safety_report")), 10
        )
        ...
        self.get_logger().info("proposal_simulation_cell_v2_10 misalignment contact batch node started")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"proposal v2.10 config not found: {path}")
    ...
    def _tick(self) -> None:
        ...
        if elapsed >= self._validation_timeout and not self._finished:
            self._write_outputs_once("misalignment_contact_gate_batch_timeout")
    ...
    def _run_sequence(self) -> None:
        ...
        elif attempted >= 5 and validated >= 5 and self._all_batch_requirements_met():
    ...
    def _status_payload(self, status: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "status": status or "misalignment_contact_gate_batch_pending",
        }
    ...
    def _write_outputs_once(self, status: str) -> None:
        ...
        self._write_json(self._output_dir / "batch_geometry_report.json", self._geometry_report)
        self._write_lines(self._output_dir / "batch_geometry_report.txt", self._dict_lines(self._geometry_report))
        self._write_csv(self._output_dir / "scenario_definition_table.csv", self._scenario_definition_rows)
        self._write_csv(self._output_dir / "scenario_pad_pose_report.csv", self._scenario_pad_rows)
        self._write_csv(self._output_dir / "scenario_ik_reachability_report.csv", self._scenario_ik_rows)
        self._write_csv(self._output_dir / "scenario_initial_no_contact_report.csv", self._initial_no_contact_rows)
        self._write_csv(self._output_dir / "task_phase_report.csv", self._phase_rows)
        self._write_csv(self._output_dir / "scenario_approach_step_report.csv", self._step_rows)
        self._write_csv(self._output_dir / "scenario_contact_transition_report.csv", self._contact_rows)
        self._write_csv(self._output_dir / "scenario_post_retreat_report.csv", self._post_retreat_rows)
        ...
        self._write_json(self._output_dir / "misalignment_contact_batch_status.json", payload)
        self._write_summary(payload)
        self._write_run_log(payload)
        self._publish_reports()
        self.get_logger().info("proposal_simulation_cell_v2_10 diagnostics written")
        rclpy.shutdown()
    ...
    def _write_summary(self, status: dict[str, Any]) -> None:
        lines = [
            ...
            f"- all_scenarios_initial_no_contact_verified: {self._bool(status['all_scenarios_initial_no_contact_verified'])}",
...
def main(args: list[str] | None = None) -> None:
    ...
    node = ProposalSimulationCellV210MisalignmentContactBatchNode()
    try:
        rclpy.spin(node)
    finally:
        if rclpy.ok():
