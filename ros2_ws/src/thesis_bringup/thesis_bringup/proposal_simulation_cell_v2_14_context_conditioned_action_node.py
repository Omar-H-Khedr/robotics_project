"""Context-conditioned guarded action validation for proposal_simulation_cell_v2_14."""
...
class ProposalSimulationCellV214ContextConditionedActionNode(Node):
    """Run deterministic context-conditioned guarded action validation in Gazebo."""

    def __init__(self) -> None:
        super().__init__("proposal_simulation_cell_v2_14_context_conditioned_action_node")
        self.declare_parameter("config_path", "")
        self.declare_parameter("output_dir", "diagnostics/proposal_simulation_cell_v2_14")
        ...
        self._output_dir = Path(
            ...
            or validation.get("output_dir", "diagnostics/proposal_simulation_cell_v2_14")
        ).expanduser()
        self._output_dir.mkdir(parents=True, exist_ok=True)

        self._simulation_engine = str(task.get("simulation_engine", "gazebo"))
        self._task_sequence_type = str(task.get("task_sequence_type", "context_conditioned_guarded_action_validation"))
        ...
        self._success_status = str(validation.get("status_success", "context_conditioned_guarded_action_validated"))
        self._partial_status = str(validation.get("status_partial", "context_conditioned_guarded_action_partially_validated"))
        self._no_contact_status = str(validation.get("status_no_contact", "context_conditioned_contact_not_triggered"))
        self._emergency_status = str(validation.get("status_safety_limit", "context_conditioned_safety_limit_exceeded"))
        self._geometry_status = str(validation.get("status_geometry_unavailable", "context_conditioned_geometry_unavailable"))
        self._ik_status = str(validation.get("status_ik_failed", "context_conditioned_ik_reachability_failed"))
        self._context_unavailable_status = str(validation.get("status_context_unavailable", "context_embeddings_unavailable"))
        ...
        self._target_name = str(target.get("target_name", "proposal_v2_14_context_conditioned_contact_pad"))
        self._target_type = str(
            target.get("contact_target_type", target.get("target_type", "computed_non_overlapping_context_conditioned_contact_pad"))
        )
        self._target_contact_topic = str(
            topic_wiring.get("raw_contact_topic", target.get("contact_topic", "/proposal_simulation_cell/context_conditioned_contact_contacts"))
        )
        self._target_gz_contact_topic = str(target.get("gz_contact_topic", ""))
        self._target_frame = str(target.get("frame_id", "context_conditioned_contact_pad"))
        ...
        self._status_pub = self.create_publisher(
            String,
            str(validation.get("status_topic", "/proposal_simulation_cell/context_conditioned_action_status")),
            10,
        )
        self._step_pub = self.create_publisher(
            String, str(validation.get("scenario_report_topic", "/proposal_simulation_cell/context_conditioned_execution_report")), 10
        )
        self._contact_pub = self.create_publisher(
            String,
            str(validation.get("contact_transition_report_topic", "/proposal_simulation_cell/context_conditioned_contact_transition_report")),
            10,
        )
        ...
        self._safety_pub = self.create_publisher(
            String, str(validation.get("safety_report_topic", "/proposal_simulation_cell/context_conditioned_safety_report")), 10
        )
        self._endpoint_pub = self.create_publisher(
            String, str(validation.get("endpoint_report_topic", "/proposal_simulation_cell/context_conditioned_endpoint_report")), 10
        )
        self._action_suggestion_pub = self.create_publisher(
            String,
            str(validation.get("action_suggestion_report_topic", "/proposal_simulation_cell/context_action_suggestion_report")),
            10,
        )
        ...
        self.get_logger().info("proposal_simulation_cell_v2_14 context conditioned action node started")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"proposal v2.14 config not found: {path}")
    ...
    def _tick(self) -> None:
        ...
        if elapsed >= self._validation_timeout and not self._finished:
            self._write_outputs_once("context_conditioned_guarded_action_timeout")

    def _run_sequence(self) -> None:
        self._record_phase("initialize", "succeeded", "context-conditioned guarded action sequence initialized", 0.0, 0.0)
        ...
        elif attempted >= 5 and validated >= 5 and self._all_batch_requirements_met() and self._all_action_parameters_within_bounds():
    ...
    def _action_suggestion_for_scenario(self, scenario_name: str) -> dict[str, Any]:
        ...
        suggestion = {
            ...
            "approach_step_distance_m": self._nominal_step_distance if is_nominal else self._offset_step_distance,
            "max_guarded_steps": self._nominal_max_guarded_steps if is_nominal else self._offset_max_guarded_steps,
    ...
    def _all_action_parameters_within_bounds(self) -> bool:
        return bool(
            ...
            and len(self._action_validation_rows) >= len(self._scenarios)
    ...
    def _run_scenario(self, scenario: dict[str, Any]) -> None:
        ...
        self._current_action_suggestion = self._action_suggestion_for_scenario(self._current_scenario_name)
        ...
        self._record_phase(self._phase_name("start"), "succeeded", "Context-conditioned scenario started", 0.0, 0.0)
    ...
    def _status_payload(self, status: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "action_suggestions_written": bool(self._action_suggestion_rows),
            "moveit_used": True,
            ...
            "status": status or "context_conditioned_guarded_action_pending",
        }
    ...
    def _write_outputs_once(self, status: str) -> None:
        ...
        self._write_csv(self._output_dir / "context_action_suggestion_table.csv", self._action_suggestion_rows)
        self._write_json(self._output_dir / "context_action_suggestion_table.json", {"rows": self._action_suggestion_rows})
        self._write_csv(self._output_dir / "context_action_parameter_validation_report.csv", self._action_validation_rows)
        self._write_csv(self._output_dir / "context_conditioned_scenario_execution_report.csv", self._scenario_execution_rows())
        self._write_csv(self._output_dir / "context_conditioned_contact_transition_report.csv", self._contact_rows)
        self._write_csv(self._output_dir / "context_conditioned_post_retreat_report.csv", self._post_retreat_rows)
        self._write_csv(self._output_dir / "context_conditioned_safety_report.csv", self._safety_rows())
        self._write_csv(self._output_dir / "context_conditioned_endpoint_report.csv", self._endpoint_rows)
        self._write_json(self._output_dir / "context_conditioned_action_manifest.json", self._action_manifest(payload))
        self._write_json(self._output_dir / "context_conditioned_action_status.json", payload)
        self._write_summary(payload)
        self._write_run_log(payload)
        self._publish_reports()
        self.get_logger().info("proposal_simulation_cell_v2_14 diagnostics written")
        rclpy.shutdown()
    ...
    def _action_manifest(self, status: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "deterministic_action_suggestion": self._deterministic_action_suggestion,
            "generated_files": [
                "context_conditioned_action_status.json",
                "context_action_suggestion_table.csv",
                "context_action_suggestion_table.json",
                "context_action_parameter_validation_report.csv",
                "context_conditioned_scenario_execution_report.csv",
                "context_conditioned_contact_transition_report.csv",
                "context_conditioned_post_retreat_report.csv",
                "context_conditioned_safety_report.csv",
                "context_conditioned_endpoint_report.csv",
                "context_conditioned_action_manifest.json",
                "summary.md",
            ],
            "learning_executed": False,
            "policy_training_executed": False,
    ...
    def _write_summary(self, status: dict[str, Any]) -> None:
        lines = [
            ...
            f"- deterministic_action_suggestion: {self._bool(status['deterministic_action_suggestion'])}",
...
def main(args: list[str] | None = None) -> None:
    ...
    node = ProposalSimulationCellV214ContextConditionedActionNode()
    try:
        rclpy.spin(node)
    finally:
        if rclpy.ok():
