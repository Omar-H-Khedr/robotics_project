...
def launch_setup(context, *args, **kwargs):
    ...
    config_path = PathJoinSubstitution(
        [FindPackageShare("thesis_bringup"), "config", "proposal_simulation_cell_v2_4.yaml"]
    )
    ...
    bridge = Node(
        ...
        name="proposal_simulation_cell_v2_4_gz_bridge",
        output="screen",
    ...
    robot_state_publisher = Node(
        ...
        name="proposal_simulation_cell_v2_4_robot_state_publisher",
        output="screen",
    ...
    spawn_robot = Node(
        package="ros_gz_sim",
        executable="create",
        name="proposal_simulation_cell_v2_4_spawn_robot",
        output="screen",
        arguments=[
            "-topic",
            "robot_description",
            "-name",
            "lbr_iisy3_r760_v2_4_moveit_execution",
            "-allow_renaming",
            "-x",
            "0",
            "-y",
    ...
    joint_state_broadcaster = Node(
        ...
        executable="spawner",
        name="proposal_simulation_cell_v2_4_joint_state_broadcaster_spawner",
        output="screen",
    ...
    joint_trajectory_controller = Node(
        package="controller_manager",
        executable="spawner",
        name="proposal_simulation_cell_v2_4_joint_trajectory_controller_spawner",
        output="screen",
    ...
    validation_node = Node(
        ...
        executable="proposal_simulation_cell_v2_4_moveit_gazebo_execution_node",
        name="proposal_simulation_cell_v2_4_moveit_gazebo_execution_node",
        output="screen",
        parameters=[
    ...
    return [
        ...
        LogInfo(
            msg=(
                ...
                "one small MoveIt-generated trajectory may execute only through the Gazebo "
                "joint_trajectory_controller. No real robot, physical endpoint, peg insertion, "
                "contact-seeking motion, learning, or scenario batch execution."
            )
        ),
        gazebo,
        bridge,
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[move_group, validation_node],
            )
        ),
        RegisterEventHandler(
            OnProcessExit(
                target_action=validation_node,
                on_exit=[EmitEvent(event=Shutdown(reason="v2.4 MoveIt Gazebo execution validation captured"))],
            )
        ),
    ]
...
def generate_launch_description():
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "output_dir",
                default_value="diagnostics/proposal_simulation_cell_v2_4",
                description="Directory for proposal simulation cell v2.4 diagnostics.",
            ),
