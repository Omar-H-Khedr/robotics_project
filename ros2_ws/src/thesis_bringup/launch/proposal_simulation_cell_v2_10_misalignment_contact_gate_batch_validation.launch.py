...
def launch_setup(context, *args, **kwargs):
    ...
    config_path = PathJoinSubstitution(
        [FindPackageShare("thesis_bringup"), "config", "proposal_simulation_cell_v2_10.yaml"]
    )
    ...
    bridge = Node(
        ...
        name="proposal_simulation_cell_v2_10_gz_bridge",
        output="screen",
    ...
    contact_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="proposal_simulation_cell_v2_10_contact_bridge",
        output="screen",
        arguments=[
            "/world/proposal_simulation_cell_v1_3_contact_physics_validation/model/"
            "proposal_v2_10_misalignment_contact_pad/link/contact_calibration_pad_link/"
    ...
    robot_state_publisher = Node(
        ...
        name="proposal_simulation_cell_v2_10_robot_state_publisher",
        output="screen",
    ...
    spawn_robot = Node(
        package="ros_gz_sim",
        executable="create",
        name="proposal_simulation_cell_v2_10_spawn_robot",
        output="screen",
        arguments=[
            "-topic",
            "robot_description",
            "-name",
            "lbr_iisy3_r760_v2_10_misalignment_contact",
            "-allow_renaming",
            "-x",
            "0",
            "-y",
    ...
    joint_state_broadcaster = Node(
        ...
        executable="spawner",
        name="proposal_simulation_cell_v2_10_joint_state_broadcaster_spawner",
        output="screen",
    ...
    joint_trajectory_controller = Node(
        package="controller_manager",
        executable="spawner",
        name="proposal_simulation_cell_v2_10_joint_trajectory_controller_spawner",
        output="screen",
    ...
    task_sequence = Node(
        ...
        executable="proposal_simulation_cell_v2_10_misalignment_contact_batch_node",
        name="proposal_simulation_cell_v2_10_misalignment_contact_batch_node",
        output="screen",
    ...
    return [
        ...
        LogInfo(
            msg=(
                ...
                "five positive-clearance misalignment contact-gate scenarios through the verified "
                "Gazebo joint_trajectory_controller. No real robot, physical endpoint, peg insertion, "
                "forceful contact, learning, or fake scenario evidence."
            )
        ),
        gazebo,
        bridge,
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[EmitEvent(event=Shutdown(reason="v2.10 misalignment contact-gate batch validation captured"))],
            )
        ),
    ]
...
def generate_launch_description():
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "output_dir",
                default_value="diagnostics/proposal_simulation_cell_v2_10",
                description="Directory for proposal simulation cell v2.10 diagnostics.",
            ),
