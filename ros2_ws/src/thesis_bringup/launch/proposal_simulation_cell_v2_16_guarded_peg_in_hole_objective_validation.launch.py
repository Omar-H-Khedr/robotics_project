...
def launch_setup(context, *args, **kwargs):
    ...
    config_path = PathJoinSubstitution(
        [FindPackageShare("thesis_bringup"), "config", "proposal_simulation_cell_v2_16.yaml"]
    )
    ...
    bridge = Node(
        ...
        name="proposal_simulation_cell_v2_16_gz_bridge",
        output="screen",
        arguments=["/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock"],
    )
    peg_contact_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="proposal_simulation_cell_v2_16_peg_contact_bridge",
        output="screen",
        arguments=["/proposal_simulation_cell/peg_contacts@ros_gz_interfaces/msg/Contacts[gz.msgs.Contacts"],
    )
    hole_contact_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="proposal_simulation_cell_v2_16_hole_contact_bridge",
        output="screen",
        arguments=["/proposal_simulation_cell/hole_contacts@ros_gz_interfaces/msg/Contacts[gz.msgs.Contacts"],
    )
    robot_state_publisher = Node(
        package="robot_state_publisher",
        executable="robot_state_publisher",
        name="proposal_simulation_cell_v2_16_robot_state_publisher",
        output="screen",
    ...
    spawn_robot = Node(
        package="ros_gz_sim",
        executable="create",
        name="proposal_simulation_cell_v2_16_spawn_robot",
        output="screen",
        arguments=[
            "-topic",
            "robot_description",
            "-name",
            "lbr_iisy3_r760_v2_16_guarded_peg_in_hole",
            "-allow_renaming",
            "-x",
            "0",
            "-y",
    ...
    joint_state_broadcaster = Node(
        ...
        executable="spawner",
        name="proposal_simulation_cell_v2_16_joint_state_broadcaster_spawner",
        output="screen",
    ...
    joint_trajectory_controller = Node(
        package="controller_manager",
        executable="spawner",
        name="proposal_simulation_cell_v2_16_joint_trajectory_controller_spawner",
        output="screen",
    ...
    task_sequence = Node(
        ...
        executable="proposal_simulation_cell_v2_16_guarded_peg_in_hole_node",
        name="proposal_simulation_cell_v2_16_guarded_peg_in_hole_node",
        output="screen",
    ...
    return [
        ...
        LogInfo(
            msg=(
                ...
                "Gazebo-only guarded peg-in-hole objective attempt with MoveIt planning checks, "
                "low-force insertion gating, and no real robot or forceful contact."
            )
        ),
        gazebo_server,
        gazebo_with_gui,
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[EmitEvent(event=Shutdown(reason="v2.16 guarded peg-in-hole objective validation captured"))],
            )
        ),
    ]
...
def generate_launch_description():
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "output_dir",
                default_value="diagnostics/proposal_simulation_cell_v2_16",
                description="Directory for proposal simulation cell v2.16 diagnostics.",
            ),
            DeclareLaunchArgument("gui", default_value="false", description="Start Gazebo with a visible GUI client."),
            DeclareLaunchArgument("headless", default_value="true", description="Compatibility flag; gui controls display."),
