...
def launch_setup(context, *args, **kwargs):
    ...
    config_path = PathJoinSubstitution(
        [
            ...
            "config",
            "proposal_simulation_cell_v1_0.yaml",
        ]
    )
    ...
    bridge = Node(
        ...
        name="proposal_simulation_cell_gz_bridge",
        output="screen",
        arguments=[
    ...
    monitor = Node(
        ...
        executable="proposal_simulation_cell_monitor",
        name="proposal_simulation_cell_v1_0_monitor",
        output="screen",
        parameters=[
    ...
    return [
        ...
        LogInfo(
            msg=(
                "proposal_simulation_cell_v1_0: Isaac Sim unavailable; "
                "using ROS 2/Gazebo-compatible fallback scene. No MoveIt, "
                "/compute_ik, real robot, controller execution, or "
                "FollowJointTrajectory is used."
            )
        ),
        gazebo,
        bridge,
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[EmitEvent(event=Shutdown(reason="simulation cell evidence captured"))],
            )
        ),
    ]
...
def generate_launch_description():
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "output_dir",
                default_value="diagnostics/proposal_simulation_cell_v1_0",
                description="Directory for proposal simulation cell diagnostics.",
            ),
